...
namespace pe {
    ...
    void fixsiz(std::vector<BYTE>& b) {
        ...
        DWORD end = 0;
        for (DWORD i = 0; i < hdr->FileHeader.NumberOfSections; i++)
            end = std::max<size_t>(end, s[i].VirtualAddress + s[i].Misc.VirtualSize);
...
namespace imp {
    ...
    std::vector<entry> find(const std::vector<BYTE>& b, const exports& map) {
        ...
        for (DWORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
            ...
            DWORD end = raw + std::min<size_t>(s[i].SizeOfRawData, s[i].Misc.VirtualSize);
    ...
    bool rebuild(std::vector<BYTE>& b, const std::vector<entry>& imports, DWORD live_irva) {
        ...
        auto place = [&](DWORD tv) -> bool {
            ...
            for (DWORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
                ...
                if (s[i].SizeOfRawData) av = std::min<size_t>(av, s[i].SizeOfRawData - (tv - s[i].VirtualAddress));
...
namespace hyp {
    ...
    void dumpcode(HANDLE h, const BYTE* base, pe::sec& s, float limit, ntflush_t flush,
        ...
        for (size_t i = 0; i < deadmap.size();) {
            size_t k = std::min<size_t>(32, deadmap.size() - i);
            std::vector<HANDLE> th;
            th.reserve(k);
            for (size_t j = 0; j < k; j++) {
                HANDLE t = CreateRemoteThread(h, nullptr, 0,
                    reinterpret_cast<LPTHREAD_START_ROUTINE>(
                        const_cast<BYTE*>(base + s.rva + deadmap[i + j])),
                    nullptr, 0, nullptr);
                if (t) th.push_back(t);
            }
            if (th.empty()) break;
            WaitForMultipleObjects(static_cast<DWORD>(th.size()), th.data(), TRUE, 500);
            for (HANDLE t : th) TerminateThread(t, 0);
            for (size_t j = 0; j < k; j++) {
                SIZE_T off = deadmap[i + j];
                SIZE_T r = mem::page(h, base + s.rva + off, std::min<SIZE_T>(pagesz, n - off),
                    b.data() + s.raw + off);
                got += r;
                if (!r) st.deadcnt++;
            }
    ...
    bool grabscan(HANDLE h, const BYTE* a, size_t len, const uint8_t nonce[12], uint32_t ctr,
        ...
        for (size_t o = 0; o < len;) {
            size_t n = std::min<size_t>(chunk, len - o);
            SIZE_T rd = 0;
...
int main(int argc, char* argv[]) {
    ...
    con::info("Roblox ( exe ) @ 0x%p  %u bytes\n", base, static_cast<unsigned>(msz));
    printf("Disk: %s\n", disk.c_str());
    ...
    pe::fixsiz(b);
    pe::runnable(b, rb);
    con::info("IMAGE_BASE fixed to 0x%p, ASLR disabled\n", base);

    con::ok("Patched " + std::to_string(st.patched) + " / " + std::to_string(st.total) +
        " bytes across " + std::to_string(st.code) + " code sections ( dead pages: " +
