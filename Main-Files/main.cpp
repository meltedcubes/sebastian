...
std::vector<entry> find(const std::vector<BYTE>& b, const exports& map) {
    ...
    for (DWORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        ...
        for (DWORD p = raw; p + 8 <= end; p += 8) {
            ...
            entry e{ it->second.first, it->second.second, v, s[i].VirtualAddress + (p - raw) };
            out.push_back(e);
            seen.insert(v);
        }
    }
...
bool rebuild(std::vector<BYTE>& b, const std::vector<entry>& imports) {
    ...
    DWORD total = util::alignup(descsz + iatsz + namesz, fa);
    ...
    auto* add = &s[nt->FileHeader.NumberOfSections];
    ZeroMemory(add, sizeof(*add));
    memcpy(add->Name, ".hetalia", 8);
    add->VirtualAddress = nrva;
    add->Misc.VirtualSize = util::alignup(total, sa);
    add->PointerToRawData = nraw;
    add->SizeOfRawData = total;
    add->Characteristics = IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE;
    nt->FileHeader.NumberOfSections++;
    nt->OptionalHeader.SizeOfImage = add->VirtualAddress + add->Misc.VirtualSize;

    BYTE* data = b.data() + nraw;
    auto* desc = reinterpret_cast<PIMAGE_IMPORT_DESCRIPTOR>(data);
    DWORD ip = descsz, np = descsz + iatsz, di = 0;

    for (const auto& [m, es] : bymod) {
        desc[di].Name = nrva + np;
        memcpy(data + np, m.c_str(), m.size() + 1);
        np += static_cast<DWORD>(m.size()) + 1;

        desc[di].FirstThunk = nrva + ip;
        auto* iat = reinterpret_cast<uint64_t*>(data + ip);
        auto* lkt = iat + es.size() + 1;
        desc[di].OriginalFirstThunk = nrva + ip + static_cast<DWORD>((es.size() + 1) * sizeof(uint64_t));
...
struct stats {
    SIZE_T patched = 0, total = 0, code = 0, data = 0, flushed = 0, dead = 0;
};
...
void dumpcode(HANDLE h, const BYTE* base, pe::sec& s, float limit, ntflush_t flush,
    ...
    SIZE_T target = static_cast<SIZE_T>(n / pagesz * limit);
    SIZE_T got = 0, fl = 0;
    ...
    printf("    code section done: %zu / %zu bytes (%zu flushed)\n", got, n, fl);
}
...
int main(int argc, char* argv[]) {
    ...
    for (auto& s : sect) {
        ...
        if (hyp::iscode(h, rb, s)) {
            ...
            hyp::dumpcode(h, rb, s, lim, flush, b, st);
        } else {
    ...
    if (imp::rebuild(b, imports)) con::ok("import directory rebuilt");
